...
def test_add_llm_responses_sync(sample_dataframe, mock_runner):
    ...
    updated_df = integrator.add_llm_responses(prompt_column="prompt_column", response_column="llm_response")
    ...
    assert pd.isna(updated_df.loc[2, "llm_response"])  # Empty prompt should not have a response
...
def test_add_llm_responses_async(sample_dataframe, mock_runner):
    ...
    updated_df = integrator.add_llm_responses(prompt_column="prompt_column", response_column="llm_response", async_mode=True)
...
def test_add_llm_responses_with_row_filter(sample_dataframe, mock_runner):
    ...
    updated_df = integrator.add_llm_responses(prompt_column="prompt_column", response_column="llm_response", row_filter=[0, 2])
...
def test_reset_responses(sample_dataframe, mock_runner):
    ...
    integrator = LLMDataFrameIntegrator(runner=mock_runner, df=sample_dataframe)
    integrator.df["llm_response"] = ["Some response", "Another response", None, "Last response"]
...
def test_add_llm_responses_creates_column_if_not_exist(sample_dataframe, mock_runner):
    ...
    updated_df = integrator.add_llm_responses(prompt_column="prompt_column", response_column="new_response_column")
