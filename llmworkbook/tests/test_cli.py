...
def sample_json(tmp_path):
    ...
    with open(json_path, "w") as f:
        f.write('[["Summarize this", "Great product", "en"], ["Translate this", "Muy bueno", "es"]]')
...
def test_cli_llm_connection(mocker):
    ...
    assert result.returncode == 0
    assert "✅ LLM Connection Successful!" in result.stdout or "❌ LLM Connection Failed!" in result.stdout
