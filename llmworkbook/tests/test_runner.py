...
async def test_call_llm_openai(mock_config):
    ...
    mock_response.choices = [{"message": {"content": "Mocked response text"}}]

    # Patch the OpenAI client's chat completion method correctly
    with patch("openai.OpenAI.chat.completions.create", return_value=mock_response):
        ...
        OpenAI.chat.completions.create.assert_called_once_with(
            model="gpt-4o-mini",
            messages=[
